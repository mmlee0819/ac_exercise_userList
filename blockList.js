...
const users = JSON.parse(localStorage.getItem('blockedGuests')) || []
let filteredUsers = []
    ...
    rawHtml += ` 
      <div class="col-sm-2 mb-4 card border border-white">
        ...
        <div class="card-footer border-white mt-0 d-flex align-items-center justify-content-between">
          <button class="btn"><i class="fa-solid fa-rectangle-xmark fa-xl" data-bs-toggle="tooltip" data-bs-placement="top" title="remove from Block List" data-id ="${item.id}"></i></button>

          <button class="btn"><i class="fa-solid fa-crown fa-xl" data-bs-toggle="tooltip" data-bs-placement="top" title="Add to VIPs" data-id ="${item.id}"></i></button>
        </div>         
      </div>
    `
  })
...
function addToVips(id) {
  console.log(id)
  const list = JSON.parse(localStorage.getItem('vipGuests')) || []
  ...
  list.push(vip)
  localStorage.setItem('vipGuests', JSON.stringify(list))
}

function removeFromBlockList(id) {
  const blockedIndex = users.findIndex((blocked) => blocked.id === id)
  // console.log(blockedIndex)
  users.splice(blockedIndex, 1)
  localStorage.setItem('blockedGuests', JSON.stringify(users))
...
dataPanel.addEventListener("click", function onPanelClicked(event) {
  if (event.target.matches(".show-user-modal")) {
    ...
    removeFromBlockList(Number(event.target.dataset.id))
